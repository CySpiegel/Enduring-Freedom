...
any mission or derivative works. There are three dependent scripts: ROS_PlaneRepair.sqf, ROS_pRepairerPlane.sqf, ROSWaveInPlane.sqf.
These scripts must be kept together.
...
params ["_pilot", "_pRepairer", "_veh", "_pweldingRod", "_pweldingCart", "_prepaircase"];
...
if ("AN2" in typeOf _veh) then {
	...
	_pRepairer setPosATL (_pRepairer modelToWorld [0.5,0,0]);
};

if ("C_Plane_Civil" in typeof _veh or "I_Plane_ION" in typeOf _veh or "C_Plane_Orbit" in typeOf _veh) then {
	[_pRepairer, "AidlPknlMstpSrasWrflDnon_G0S"] remoteExec ["switchMove", 0];
};
...
while {!ROSPlaneRepaired} do {
	...
	if (ROSPlaneRepaired) exitWith {
		...
		{if (typeOf _x == "#particlesource") then {deleteVehicle _x}} forEach (_pRepairer nearObjects 7);
	};
