...
ROS Heli repair v1.5 - by RickOShay
...
if (abs ((getdir _nHeliPad +180) - (getdir _hRepairer))>15) then {
	_nHeliPad setdir (getdir _hRepairer -180);
};

// If unit is too close to helipad move back 20m relative to helipad.
if (_hRepairer distance _nHeliPad <20) then {
	_dir = getDir _hRepairer;
	_pos = _nHeliPad getPos [-20, _dir-10];
	_hRepairer setPosATL _pos;
	_hRepairer setdir _dir;
	_initPos = getPosATL _hRepairer;
	_initDir = getdir _hRepairer;
};
...
ROS_Repair_heli_fnc_completed = false;
publicVariable "ROS_Repair_heli_fnc_completed";

ROS_Repair_heli_fnc = {
	params ["_hRepairer", "_veh", "_initPos", "_initDir", "_hitpartType", "_hitpartText", "_nHeliPad", "_addRefuelItems", "_ftruck", "_wlight"];
	...
	if (isplayer _pilot && vehicle _pilot == _veh) then {
		[_hRepairer,"You must exit the vehicle."] remoteExec ["sidechat", _pilot];
		["You must exit the vehicle."] remoteExec ["hint", _pilot];
	...
	// Clean up
	if (_addRefuelItems && !isNull Hhose1) then {
	...
	ROS_Repair_heli_fnc_completed = true;
	publicVariable "ROS_Repair_heli_fnc_completed";
...
while {true} do {
	...
	if (!isNull _nearestHeli && alive _nearestHeli) then {
		...
		if (isTouchingGround _nearestHeli) then {
			...
			if (isObjectHidden _wlight && _addRefuelItems) then {_wlight hideObjectGlobal false;};

			// Prepare fuel pipes
			[] call ROS_HeliFuelPipes_Fnc;

			// Start repair if fuel (<0.9) or vehicle damage (any hp>0.1)
			_initDir = _hRepairer getdir _nearestHeli;
			if (_nearestHeli distance _nHeliPad <= _pDist && (_heliFuel <0.9 or _heliDamage >=0.1)) then {
				if (isplayer (driver _nearestHeli)) then {
					...
					sleep 1;
					[_hRepairer, _nearestHeli, _initPos, _initDir, _hitpartType, _hitpartText, _nHeliPad, _addRefuelItems, _ftruck, _wlight] spawn ROS_Repair_heli_fnc;
					_ctl = false;
			...
			waitUntil {ROS_Repair_heli_fnc_completed};
